...
namespace GraphicsProgramming
{
	class Lesson5 : Lesson
	{
		...
		private Model cube, sphere,  lantern;

		private Vector3[] lanternPositions = new Vector3[20];
		...
		{
			effect = Content.Load<Effect>("Effects/Lesson5");
			...
			lantern = Content.Load<Model>("TerrainResources/streetlamp");
			foreach (ModelMesh mesh in lantern.Meshes)
			{
				foreach (ModelMeshPart meshPart in mesh.MeshParts)
				{
					meshPart.Effect = effect;
			...
			rt = new RenderTarget2D(graphics.GraphicsDevice, graphics.PreferredBackBufferWidth, graphics.PreferredBackBufferHeight, false, graphics.PreferredBackBufferFormat, graphics.PreferredDepthStencilFormat);
		}
		...
		private void GeneratePlane(float gridSize = 8.0f, float height = 128f)
		{
			...
			for (int i = 0; i < lanternPositions.Length; i++)
			{
				...
				int randVert = rand.Next(0, vertices.Length);

				lanternPositions[i] = new Vector3(vertices[randVert].Position.X/* * 40*/, vertices[randVert].Position.Y, vertices[randVert].Position.Z);
			}
		}
		...
		public override void Draw(GameTime gameTime, GraphicsDeviceManager graphics, SpriteBatch spriteBatch)
		{
          ...
          //  effect.Parameters["Ambient"].SetValue(new Vector3(.25f, .20f, .15f));
			...
			/**effect.CurrentTechnique = effect.Techniques["Lantern"];
			RenderModel(lantern, World * Matrix.CreateScale(200));*/


            foreach (Vector3 lanternPos in lanternPositions)
            {
                effect.CurrentTechnique = effect.Techniques["Lantern"];
                RenderModel(lantern, World * Matrix.CreateScale(5) * Matrix.CreateTranslation(lanternPos));
            }
